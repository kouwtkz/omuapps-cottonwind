<script lang="ts">
    ...
    import { TUTORIAL_APP2, TUTORIAL_ASSET_APP2, TutorialApp } from ".";

    // APIを触るのに必要なOmuオブジェクトを生成します
    const omu = new Omu(TUTORIAL_APP2);
    ...
    async function handleInstall() {
        ...
        const session = await omu.sessions.generateToken({
            app: TUTORIAL_ASSET_APP2,
        });
        const url = new URL(TUTORIAL_ASSET_APP2.url!);
